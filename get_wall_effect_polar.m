...
%     A=axes;
    ...
    S=polarplot3d(Mzz,AL,'AngularRange',[-pi/2 3*pi/2],'plottype','surfa','RadialRange',r_v,'AxisLocation','off','MeshScale',1*[1 1]);
%     S=surf(X,Y,zeros(size(Z)),Z,'LineStyle','none','FaceColor','interp');
    ...
    colormap(gca,'parula');
    set(gca,'CLim',[-.65 .25],'Alim',[0 1.25]);%,'XDir','reverse');
    set(gcf,'Color',bgcol);
