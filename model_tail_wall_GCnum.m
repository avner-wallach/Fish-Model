function [out_strct]=model_tail_wall_GCnum(dbname)
%% params
% params.phi0=.75*pi;    
...
% nhidden=100;
nettype='GC';
nhidden_v=[100 300 1000 3000 10000];
...
%     fb_data=repmat(data0,K_tail,1); %filtered fb
    ...
    inputs={[tail data],[tail fb_data]};
    for nh=1:numel(nhidden_v)
        nhidden=nhidden_v(nh);
        for k=1:numel(inputs)
            [ net,ex_model(nh,k+1).rsq,ex_model(nh,k+1).sig ] = ni_network_new(inputs{k}(ind,:)',zre(ind)',nettype,[],nhidden);
            z_ni=-net(inputs{k}')';
            z_ni(indnan)=nan;
            [ex_model(nh,k+1).ni_Re,ex_model(nh,k+1).ni_Ex,ex_model(nh,k+1).N]=get_maps(tail,z_ni);
            z_out=z+z_ni;
            [ex_model(nh,k+1).out_Re,ex_model(nh,k+1).out_Ex]=get_maps(tail,z_out);
            ex_model(nh,k+1).total_re_flat=nansum(abs(ex_model(nh,k+1).out_Re(:)));
            ex_model(nh,k+1).var_re_flat=nanvar(ex_model(nh,k+1).out_Re(:));
            ex_model(nh,k+1).total_re_weighted=nansum(abs(ex_model(nh,k+1).out_Re(:).*ex_model(nh,k+1).N(:)))/nansum(ex_model(nh,k+1).N(:));    
        end

        [ex_model(nh,1).out_Re,ex_model(nh,1).out_Ex,ex_model(nh,1).N]=get_maps(tail,z);
        ex_model(nh,1).total_re_flat=nansum(abs(ex_model(nh,1).out_Re(:)));
        ex_model(nh,1).var_re_flat=nanvar(ex_model(nh,1).out_Re(:));
        ex_model(nh,1).total_re_weighted=nansum(abs(ex_model(nh,1).out_Re(:).*ex_model(nh,1).N(:)))/nansum(ex_model(nh,1).N(:));    
